...
{  
		...
		internal pure returns(uint256)
	{
		...
		bytes memory ring_link = abi.encodePacked(x, y, uint256(uint160(hashed_point)), message);

		return uint256(keccak256(ring_link));
	}


	function Verify( uint256[] memory pubkeys, uint256[] memory tees, uint256 seed, uint256 message )
